<template>
  <div
    class="cal:mx-auto cal:h-full cal:min-h-[524px] cal:bg-white cal:px-[20px] cal:dark:bg-theme-900 cal:sm:mx-0"
    :class="[config.compact ? 'cal:w-[330px] cal:rounded-xl cal:sm:w-[400px]' : 'cal:w-[330px] cal:rounded-r-xl cal:sm:w-[440px]', config.profileImage ? 'cal:pt-4' : 'cal:pt-10']"
  >
    <PrimaryButton v-if="config.compact === true" class="cal:mb-8 cal:mt-5" @click="goPickingDates">
    ...
    <div v-if="!state.loading" class="cal:h-full">
      <Transition
        ...
        mode="out-in"
      >
        <div v-if="state.timeSlots && state.timeSlots.length > 0 && !state.loading" class="cal:flex cal:h-full cal:flex-col cal:justify-between">
          <div>
            <div :class="[(config.locale?.texts?.introduction || config.profileImage) && !reservation && !config.compact ? 'cal:pt-[54px]' : 'cal:pt-0']">
              <p class="cal:text-2xl cal:font-semibold cal:tracking-tighter cal:text-theme-600 cal:dark:text-theme-300">
                {{ getFormattedDayInMonth(state.timeSlots[0].start) }}
              </p>
            </div>
            <div class="cal:pb-0.5 cal:pt-6">
              <p v-if="config.locale && config.locale.texts?.choosePreferredTime" class="cal:text-base cal:font-medium cal:tracking-tighter cal:text-theme-600 cal:dark:text-theme-200">
                {{ config.locale.texts?.choosePreferredTime }}
              </p>
            </div>

            <div class="cal:mt-2 cal:overflow-y-auto cal:px-1" :class="[config.locale?.hideTimePreferences ? 'cal:h-[375px]' : 'cal:h-[345px]']">
              <div v-for="(timeSlot, i) in state.timeSlots" :key="i" class="w-full">
                <ConfirmationButton :time-slot="timeSlot" @select-time-slot="select(timeSlot)"></ConfirmationButton>
              </div>
            </div>
          </div>

          <div v-if="!config.locale?.hideTimePreferences" class="cal:mt-5 cal:flex cal:items-center cal:gap-2 cal:pb-4 cal:sm:mt-0">
        ...
        <div v-else class="cal:w-full">
          <div v-if="!state.monthHasTimeSlots" class="cal:flex cal:h-full cal:flex-col cal:items-center cal:justify-center">
            <div class="cal:mt-24 cal:flex cal:h-full cal:items-center cal:justify-center">
              <div class="cal:dark:text-themered-50 cal:flex cal:h-40 cal:w-40 cal:items-center cal:justify-center cal:rounded-2xl cal:px-5">
                <p v-if="config.locale && config.locale.texts?.noTimeSlotAvailable" class="cal:dark:text-theme-theme cal:text-center" v-html="config.locale.texts?.noTimeSlotAvailable"></p>
              </div>
            </div>
          </div>
          <div v-else class="cal:flex cal:flex-col cal:items-center cal:justify-center">
            <div class="cal:mt-48 cal:flex cal:h-full cal:items-center cal:justify-center">
              <div class="cal:flex cal:w-40 cal:flex-col cal:items-center cal:justify-center cal:rounded-2xl cal:px-7 cal:py-5 cal:text-theme-700 cal:dark:text-theme-200">
                ...
                <p v-if="config.locale && config.locale.texts?.chooseDate" class="cal:mt-2 cal:w-40 cal:text-center cal:text-theme-900 cal:dark:text-theme-200" v-html="config.locale.texts?.chooseDate"></p>
              </div>
            </div>
          </div>
        </div>
