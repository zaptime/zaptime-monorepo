<template>
    <div v-if="!state.loading" class="cal-grid cal-grid-cols-7 cal-gap-y-1">
      <div class="cal-flex mt-2 cal-justify-center" v-for="(day, i) in state.days" :key="i">
        <div class="cal-flex cal-items-center cal-justify-center">
          ...
          " :class="{
            ...
            }" class="cal-w-10 cal-h-10 cal-text-sm cal-rounded-md" @click.prevent="dayClickedLocalProxy(day)">
            {{ day.label }}
          </button>
        </div>
      </div>
