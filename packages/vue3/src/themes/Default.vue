    ...
    <Calendar class="basic-theme cal-h-full">
      ...
      <template #header>
        <div
          ...
          :key="header"
          class="cal-flex cal-justify-center cal-text-[11px] cal-font-medium cal-text-theme-700 dark:cal-text-theme-300"
        >
          {{ header }}
        </div>
      </template>
            ...
            <div class="cal-flex cal-items-center cal-justify-center">
              <button
                ...
                :class="{
                  ...
                  'cal-cursor-not-allowed dark:cal-text-theme-400':
                    (day.isPast && !day.isCurrentMonth) ||
                    (!dayHasTimeSlot(day) && day.isCurrentMonth),
                  'cal-bg-theme-500 cal-font-semibold cal-text-theme-50 dark:cal-bg-theme-25 dark:cal-text-theme-700 dark:hover:cal-text-theme-600':
                    isSelectedDay(day),
                  'cal-transition-color cal-duration-100 dark:cal-text-theme-100':
                    !isSelectedDay(day) && dayHasTimeSlot(day) && !day.isPast,
                  'cal-pointer-events-none cal-text-theme-400 dark:cal-text-theme-400':
                    day.isPast,
                }"
      ...
      <template #default>
        <button
          class="cal-ml-4 cal-mt-5 cal-rounded-md cal-bg-theme-700 cal-px-6 cal-py-3.5 cal-text-theme-100"
        ...
        >
          <div v-if="!state.loading">
            <div
              v-if="
                state.timeSlots && state.timeSlots.length > 0 && !state.loading
              "
            >
              <div class="cal-pt-[54px]">
                <p
                  class="cal-text-[32px] cal-font-semibold cal-tracking-tighter cal-text-theme-600 dark:cal-text-theme-300"
                >
              ...
              <div class="cal-mt-2 cal-h-40 cal-overflow-y-auto cal-px-1">
                ...
                >
                  <p
                    class="cal-leading-4"
                    :class="{
                      'cal-text-theme-700 dark:cal-text-white':
                        isSelected(timeSlot),
                      'cal-text-theme-800 dark:cal-text-theme-100':
                        !isSelected(timeSlot),
                    }"
                  >
            ...
            <div v-else class="cal-w-full">
              ...
              >
                <div
                  class="cal-mt-24 cal-flex cal-h-full cal-items-center cal-justify-center"
                >
                  <div
                    class="dark:cal-text-themered-50 cal-flex cal-h-40 cal-w-40 cal-items-center cal-justify-center cal-rounded-2xl cal-px-5"
                  >
                    <p
                      v-if="
                        config.locale &&
                        config.locale.texts?.noTimeSlotAvailable
                      "
              ...
              >
                ...
                >
                  <div
                    class="cal-flex cal-w-40 cal-flex-col cal-items-center cal-justify-center cal-rounded-2xl cal-px-7 cal-py-5 dark:cal-text-theme-200"
                  >
                    <svg
